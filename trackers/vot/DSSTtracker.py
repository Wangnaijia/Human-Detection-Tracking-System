...
import utils
import trackers.vot
from trackers.vot.vot import pyhog
...
class DSSTtracker:
    ...
    def track(self, image):
        ...
        self.bbox = [self.pos[1] - self.target_size[1] / 2,
                     self.pos[0] - self.target_size[0] / 2,
                     self.target_size[1],
                     self.target_size[0]
                     ]
        self._results.append(self.bbox)
        return trackers.vot.Rectangle(self.pos[1] - self.target_size[1] / 2,
                                      self.pos[0] - self.target_size[0] / 2,
                                      self.target_size[1],
                                      self.target_size[0]
                                      )
    ...
    def quit(self):
        # saving results
        with open('output_dsst.txt', 'w') as f:
